...
const WeatherIconCodes: Record<string, number[]> = {
  ...
  RemixIcon: Record<string, RemixiconComponentType> = {
    cloud: RiCloudFill,
    snow: RiSnowyFill,
    storm: RiThunderstormsFill,
    sun: RiSunFill,
    wind: RiWindyFill,
    rain: RiRainyFill,
  },
  WeatherLookup: { [key: string]: string } = {
    cloud: "Cloudy",
    snow: "Snowy",
    storm: "Storm",
    sun: "Sunny",
  ...
  ChangeContext = (key: string) => WeatherLookup[key] || "Sunny",
  WeatherContext: Record<number, string> = {
    "-1": "",
...
function FindRemixIconCodes(code: number): [string, RemixiconComponentType] {
  for (const [key, codes] of Object.entries(WeatherIconCodes)) {
    if (codes.includes(code)) {
      return [ChangeContext(key), RemixIcon[key]];
    }
  }
  return ["sunny", RemixIcon["sun"]];
}

export { FindWeatherIconCodes, WeatherContext, FindRemixIconCodes };
